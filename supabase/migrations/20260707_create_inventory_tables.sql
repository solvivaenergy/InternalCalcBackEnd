...
create table if not exists public.device_settings (
    ...
    day_start_hour integer not null default 6 check (
...
insert into
    public.device_settings (id, day_start_hour)
values (true, 6) on conflict (id) do
update
set
...
values ('single', 630, 8600, 1.3),
    ('three', 650, 9000, 1.6) on conflict (phase) do
update
set
...
values ('single', 5.000, 58384, 1),
    ...
    ('three', 50.000, 537081, 6) on conflict (phase, rated_kw) do
update
set
...
create policy device_settings_read_all on public.device_settings for
select using (true);

drop policy if exists devices_read_all on public.devices;

create policy devices_read_all on public.devices for
select using (true);

drop policy if exists panel_settings_read_all on public.panel_settings;

create policy panel_settings_read_all on public.panel_settings for
select using (true);

drop policy if exists inverters_read_all on public.inverters;

create policy inverters_read_all on public.inverters for
select using (true);
...
grant
select on public.device_settings, public.devices, public.panel_settings, public.inverters to anon, authenticated;

grant
update on public.device_settings,
public.devices,
public.panel_settings,
public.inverters to authenticated;
